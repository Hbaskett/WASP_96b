        ...
        for metallicity in ["solar","10xsolar"]:
            ...
            for t in time_step:
                ...
                for gas in tqdm(gases):
                    vrbls[planet][exp][metallicity][t][gas] = {}
                    # Find other data
        ...
        for gas in tqdm(gases):
            ...
            for exp, plt_kw in leg_line_kw.items():
                for lon, clr, clr60 in zip(tgt_lons, lon_clrs, lon_clrs_60):
                    vp00 = vrbls[planet][exp][metallicity][t][gas]["species_plevs_locs"].extract(
                        iris.Constraint(latitude=0, longitude=lon)
                    )
                    vp60 = vrbls[planet][exp][metallicity][t][gas]["species_plevs_locs"].extract(
            ...
            ax.set_title(f"For {metallicity} metallicity mole fraction of {gas} at day {day}")
            ...
            output_dir = Path.home() / "um" / "um_runs" / "chem_lats" / mdh / gas
